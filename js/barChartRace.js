function _1(md) {
  return md`
# Bar Chart Race
## Total Cost of each Airport by Year (1990 - 2002)
  `;
}

function _data(FileAttachment) {
  return FileAttachment("../data/birdstrike_1.csv").csv({ typed: true });
}
...
function _names(data) {
  return new Set(data.map((d) => d["Airport Name"]));
}
...
function _bars(n, color, y, x, prev, next) {
  return function bars(svg) {
    let bar = svg.append("g").attr("fill-opacity", 0.9).selectAll("rect");
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      "../data/birdstrike_1.csv",
      {
        url: new URL("../data/birdstrike_1.csv", import.meta.url),
        mimeType: "text/csv",
        toString,
      },
    ],
  ...
  main
    ...
    .define(
      "axis",
      ["margin", "d3", "x", "width", "tickFormat", "barSize", "n", "y"],
      _axis
    );
  main
  ...
  main
    .variable(observer("y"))
    .define("y", ["d3", "n", "margin", "barSize"], _y);
  main
    .variable(observer("height"))
    .define("height", ["margin", "barSize", "n"], _height);
